...
hurricane_data <- read.csv("atlantic_updated.csv", header = TRUE)

# The date and time data is a mess. Let's reformat it into one field:

hurricane_data$Year <- substr(as.character(hurricane_data$Date), 1,4)
...
hurricane_data.new <- hurricane_data[hurricane_data$Year >= 1900,]
...
cruft <- c("Date", "dateString", "Event", "Status", "Category", "Minimum.Pressure",  "Low.Wind.NE", "Low.Wind.SE","Low.Wind.SW", "Low.Wind.NW", "Moderate.Wind.NE", "Moderate.Wind.SE","Moderate.Wind.SW", "Moderate.Wind.NW", "High.Wind.NE", "High.Wind.SE","High.Wind.SW", "High.Wind.NW",  "Day")

trimmed_data <- hurricane_data.new[,!(names(hurricane_data) %in% cruft)]
...
# storm_landfall<-storm_landfall[m.dat
...
# Plot a boxplot of the maximum wind speed at landfall
